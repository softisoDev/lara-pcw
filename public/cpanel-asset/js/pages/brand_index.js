            ...
            buttons: [
                {
                    ...
                    action: function (e, dt, node, config) {
                        ...
                        window.location.href = addSlash2Url(route('admin.brand.show', {id: dataID}));
                    }
                },
                {
                    ...
                    action: function (e, dt, node, config) {
                        ...
                        window.location.href = addSlash2Url(route('admin.brand.edit', {id: dataID}));
                    }
                },
                {
                    ...
                    action: function (e, dt, node, config) {
                        let get_row = table.rows({selected: true}).nodes();
                        let dataID = $(get_row[0]).data('id');
                        removeData(addSlash2Url(route('admin.brand.destroy', {id: dataID}), dataID));
                    }
                }
            ],
        });
...
function initTrashedTable() {
    let trashedTable = $('#trash-table').DataTable({
        ...
        serverSide: true,
        "ajax": {
            url: addSlash2Url(route('admin.brand.datatable.trashed')),
            dataType: "JSON",
            type: "POST",
                ...
                buttons: [
                    {
                        ...
                        action: function (e, dt, node, config) {
                            ...
                            let dataID = $(get_row[0]).data('id');
                            restoreData(addSlash2Url(route('admin.brand.restore', {id: dataID}), dataID));
                        }
                    },
                    {
                        ...
                        action: function (e, dt, node, config) {
                            let get_row = trashedTable.rows({selected: true}).nodes();
                            let dataID = $(get_row[0]).data('id');
                            removeDataPermanently(addSlash2Url(route('admin.brand.datatable.destroy.permanently', {id: dataID}), dataID));
                        }
                    }
                ],
            },
