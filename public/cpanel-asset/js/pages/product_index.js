...
function initTrashedTable() {
    let trashedTable = $('#trash-table').DataTable({
        ...
        serverSide: true,
        "ajax": {
            url: addSlash2Url(route('admin.product.datatable.trashed')),
            dataType: "JSON",
            type: "POST",
                ...
                buttons: [
                    {
                        ...
                        action: function (e, dt, node, config) {
                            ...
                            let dataID = $(get_row[0]).data('id');
                            restoreData(addSlash2Url(route('admin.product.restore', {id: dataID}), dataID));
                        }
                    },
                    {
                        ...
                        action: function (e, dt, node, config) {
                            let get_row = trashedTable.rows({selected: true}).nodes();
                            let dataID = $(get_row[0]).data('id');
                            removeDataPermanently(addSlash2Url(route('admin.product.datatable.destroy.permanently', {id: dataID}), dataID));
                        }
                    }
                ],
            },
